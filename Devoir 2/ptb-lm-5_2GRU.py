...
from models_5_2 import RNN, GRU
...
parser.add_argument('--data', type=str, default='../content/gdrive/My Drive/Colab Notebooks/IFT6135/Devoir 2/data',
...
def run_epoch(model, data, is_train=False, lr=1.0):
    ...
    for step, (x, y) in enumerate(ptb_iterator(data, model.batch_size, model.seq_len)):
        ...
        if args.model == 'TRANSFORMER':
            ...
            outputs, hidden, hid_grad = model(inputs, hidden,tt)
...
#        losses.append(costs)
        iters += model.seq_len
        #loss_per_time.append(loss.data.item())
        
#        if (step+1)%35 == 0:
#            if args.model != 'TRANSFORMER':
#                hidden = model.init_hidden()
#                losses=losses+(np.array(loss_per_time))
#                loss_per_time=[]
#                torch.cuda.empty_cache()
#                
#            else :
#                if step == 34:print("Taking the loop")
#                losses=losses+(np.array(loss_per_time))
#                loss_per_time=[]
#                torch.cuda.empty_cache()
        ...
        if args.debug:
            print(step, loss)
        if not is_train:  # Only update parameters if training 
            #loss.backward()
            print("1 minibatch")
...
#    train_ppl, train_loss = run_epoch(model, train_data, True, lr)
    ...
    # RUN MODEL 1 training minibatch
    grad_time_2 = run_epoch(model, train_data)
