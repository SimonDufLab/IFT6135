...
def calculate_fid_score(sample_f, test_f):
    """
    Inputs : Iterators over the sample features and the test features. The features are in a np array
    
    Output : The FID distance, calculate over the mean and covariance of the features
    
    Notes : Some material is borrowed from https://github.com/mseitzer/pytorch-fid/blob/master/fid_score.py
            more precisely, the way they calculate the square root matrix...
    """
    
    s_features,t_features = 0,0
    for (x,y) in zip(sample_f,test_f):
        if type(s_features) is int:
            s_features = x
            t_features = y
            #print(type(s_features))
        else:
            s_features = np.column_stack((s_features,x))
            t_features = np.column_stack((t_features,y))
    ...
    s_mu = np.mean(s_features,axis=1)
    s_Sigma = np.cov(s_features)
    s_Sigma = s_Sigma 
    
    t_mu = np.mean(t_features,axis=1)
    t_Sigma = np.cov(t_features)
    t_Sigma = t_Sigma 
    
    #Fid calculation, and eliminate very small imaginary part    

    Fid= ((s_mu-t_mu)**2).sum() + np.trace(s_Sigma) + np.trace(t_Sigma) - 2*np.trace(linalg.sqrtm(np.dot(s_Sigma,t_Sigma)))
    return Fid.real
